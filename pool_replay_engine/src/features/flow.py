...
def add_flow_features(flow_rows: list[dict], price_rows: list[dict]) -> list[dict]:
    ...
    for code, series in by_code.items():
        ...
        for i, r in enumerate(series):
            # Use net_mf_amount from ods_moneyflow table
            r["flow_1d"] = r.get("net_mf_amount", 0)
            r["flow_3d"] = sum(x.get("net_mf_amount", 0) for x in series[max(0, i - 2): i + 1])
            r["flow_5d"] = sum(x.get("net_mf_amount", 0) for x in series[max(0, i - 4): i + 1])
            amount = amount_map.get((code, r["trade_date"]), 0) or 1
            r["flow_ratio"] = r.get("net_mf_amount", 0) / amount
